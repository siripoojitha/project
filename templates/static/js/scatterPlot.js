var origin = [480, 300], j = 10, scale = 20, scatter = [], yLine = [], xGrid = [], 
    beta = 0, alpha = 0, key = function(d){ return d.id; }, startAngle = Math.PI/4;
var svg    = d3.select('svg').call(d3.drag().on('drag', dragged)
                .on('start', dragStart).on('end', dragEnd)).append('g');
...
function d3init(points=null){
    ...
    if(points !== null){
        scatter = points.map(([x,y,z])=>({x:x*10,y:y*10,z:z*10}));
    }
    var data = [
        grid3d(xGrid),
        point3d(scatter),
